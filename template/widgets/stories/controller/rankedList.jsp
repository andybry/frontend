 ...
 * $URL:$
 *
 * This is the controller for the rankedListTeaser view of stories widget
...
<jsp:useBean id="rankedController" class="uk.co.trinitymirror.widgets.stories.RankedListController" scope="page"/>
...
<c:set target="${stories}" property="source" value="${fn:trim(widgetContent.fields.sourceRankedList.value)}"/>
<c:set target="${stories}" property="begin" value="${fn:trim(widgetContent.fields.beginRankedList.value) - 1}"/> 
<c:set target="${stories}" property="numberOfItems"
       value="${fn:trim(widgetContent.fields.numberOfItemsRankedList.value)}"/>
<c:set target="${stories}" property="useShortHeadline" value="${fn:trim(widgetContent.fields.useShortHeadline.value)}" />
...
<section:use uniqueName="${fn:trim(widgetContent.fields.helpSectionUniqueName.value)}">
...
<c:if test="${isAjaxCallback or isInESI}">
  ...
  <c:set target="${stories}" property="numberOfItems" value="${fn:trim(widgetContent.fields.numberOfItemsForRummbleRankedList.value)}" />
</c:if>
<c:choose>
...
<c:if test="${not empty stories.articles}">
  ...
  <c:choose>
    <c:when test="${(isInESI or isAjaxCallback) and (not empty stories.enableRummbleLabs and stories.enableRummbleLabs)}">
      ...
      <c:if test="${not empty userId}">
        ...
        <c:forEach var="item" items="${stories.articles}">
          ...
          <c:set var="maxAgeOfArticlesInHours" value="${fn:trim(widgetContent.fields.maxAgeRankedList.value)}" />
      ...
      <%-- filter the ordered article list by count --%>
      <jsp:useBean id="limitedOrderedList" class="java.util.ArrayList" scope="page" />
      <c:forEach var="item" items="${orderedArticleList}" begin="0" end="${numberToOutput - 1}">
        <collection:add collection="${limitedOrderedList}" value="${item}" />
      </c:forEach>
      <c:set target="${stories}" property="articles" value="${limitedOrderedList}"/>
    ...
    <c:otherwise>
      ...
      <c:set var="ajaxUrl" value="${ajaxUrl}?service=ajax&item=${fn:trim(widgetContent.fields.ajaxUrlParamRankedList.value)}"/>
      <jsp:include page="helpers/calculateViewModel.jsp" />
